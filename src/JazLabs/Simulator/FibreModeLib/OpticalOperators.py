...
import JazLabs.Simulator.FibreModeLib.GaussianBeamBasis as GaussianFuncs
import JazLabs.Simulator.FibreModeLib.FittingFunction as FitFuncs
import JazLabs.Simulator.FibreModeLib.GeneralFunctions as GenFuncs

# import scipy
from scipy.fft import fft, fftfreq, fftshift, fft2,ifft2,rfft2,irfft2


def circular_aperture(X, Y, radius, center_x=0.0, center_y=0.0):
    """
    Generates a circular aperture mask.

    Parameters
    ----------
    X, Y : 2D arrays
        Coordinate grids (usually from np.meshgrid)
    radius : float
        Radius of the aperture (in same units as X, Y)
    center_x : float
        X coordinate of the aperture centre
    center_y : float
        Y coordinate of the aperture centre

    Returns
    -------
    aperture : 2D float array
        1 inside the aperture, 0 outside
    """
