...
import JazLabs.utils.PlotingFunctions as cmplxplt
# Global Ploting properties and style
plt.style.use('dark_background')
plt.rcParams['figure.figsize'] = [10,10]

class AllMetricValues:
    def __init__(self,CouplingMatrix,SingularValues,DiagCoup,OffDiagCoup,IL,MDL,VolumePres,overlapSum,Visibility,DiagPwrSum,SNR):
        self.CouplingMatrix=CouplingMatrix
        self.SingularValues=SingularValues
        self.DiagCoup=DiagCoup
        self.OffDiagCoup=OffDiagCoup
        self.IL=IL
        self.MDL=MDL
        self.VolumePres=VolumePres
        self.overlapSum=overlapSum
        self.Visibility=Visibility
        self.DiagPwrSum=DiagPwrSum
        self.SNR=SNR

def CalculateCouplingMatrix(fieldX,fieldY,pixelsize):
    # Dims=np.shape(fieldIn)
    # modeCount=Dims[0]
    # Ny=Dims[1]
    # Nx=Dims[2]
    
    Dims=np.shape(fieldX)
    modeCountX=Dims[0]
    Ny=Dims[1]
    Nx=Dims[2]
    
    Dims=np.shape(fieldY)
    modeCountY=Dims[0]
    Ny=Dims[1]
    Nx=Dims[2]
    ...
    CoupMat=np.zeros((modeCountY,modeCountX),dtype=complex)
    ...
    for imodeY in range(modeCountY):
        for imodeX in range(modeCountX):
            ...
            # CoupMat[imodeY,imodeX]=np.sum(np.conj(fieldY[imodeY,:,:])*((fieldX[imodeX,:,:])))*dxy
            # CoupMat_norm[imodeY,imodeX]=np.abs(np.sum((fieldY[imodeY,:,:])*((fieldX[imodeX,:,:])))\
            # /(np.sqrt(np.sum(np.abs(fieldY[imodeY,:,:])**2)*(np.sum(np.abs(fieldX[imodeX,:,:])**2)))))**2
        
    return CoupMat
def CalculateCouplingMatrix_WithNorm(fieldX,fieldY,pixelsize):
    # Dims=np.shape(fieldIn)
    # modeCount=Dims[0]
    # Ny=Dims[1]
    # Nx=Dims[2]
    ...
    for imodeY in range(modeCountY):
        for imodeX in range(modeCountX):
            ...
            # CoupMat[imodeY,imodeX]=np.sum((np.conj(fieldX[imodeX,:,:]))*(fieldY[imodeY,:,:]))*dxy
            # CoupMat[imodeY,imodeX]=np.sum(np.conj(fieldY[imodeY,:,:])*((fieldX[imodeX,:,:])))*dxy
            CoupMat_norm[imodeY,imodeX]=(np.sum(np.conj(fieldY[imodeY,:,:])*((fieldX[imodeX,:,:])))\
            /(np.sqrt(np.sum(np.abs(fieldY[imodeY,:,:])**2)*(np.sum(np.abs(fieldX[imodeX,:,:])**2)))))
        
    return CoupMat,CoupMat_norm

def CalculateILAndMDL(CoupMat):
    u, s, vh=np.linalg.svd(CoupMat, full_matrices=True,compute_uv=True)
    s=s/s[0]
    # s = s**2;
    VolumePres=np.prod(s)
    IL=abs(10*np.log10(np.mean(s**2)));
    MDL=abs(10*np.log10(s[-1]/s[0]));
   
    return IL,MDL,VolumePres,s
...
def CalculateSNR(CoupMat):
    ...
    modeCountY=Dims[0]
    modeCountX=Dims[1]
    matdtype=CoupMat.dtype
    
    modeCount_Diag=max([modeCountY,modeCountX])
    
    DiagCoup=np.zeros(modeCount_Diag,dtype=matdtype)
    OffDiagCoup=np.zeros(((modeCountY*modeCountX)-modeCount_Diag),dtype=matdtype)
    DiagPWR_sum=0
    OffDiagPWR_sum=0
    imodeOffDiag=0
    imodeDiag=0
    for imodeY in range(modeCountY):
        for imodeX in range(modeCountX):
            if (imodeY==imodeX):
                DiagCoup[imodeDiag]=CoupMat[imodeY,imodeX]
                if (matdtype==complex):
                    DiagPWR_sum=DiagPWR_sum+np.abs(CoupMat[imodeY,imodeX])**2
                else:
                    DiagPWR_sum=DiagPWR_sum+(CoupMat[imodeY,imodeX])
                imodeDiag=imodeDiag+1
            else:
                OffDiagCoup[imodeOffDiag]=CoupMat[imodeY,imodeX]
                if (matdtype==complex):
                    OffDiagPWR_sum=OffDiagPWR_sum+np.abs(CoupMat[imodeY,imodeX])**2
                else:
                    OffDiagPWR_sum=OffDiagPWR_sum+(CoupMat[imodeY,imodeX])
                # OffDiagPWR_sum=OffDiagPWR_sum+np.abs(CoupMat[imodeY,imodeX])**2
                imodeOffDiag=imodeOffDiag+1
                
    SNR=(10*np.log10(DiagPWR_sum/OffDiagPWR_sum));
    DiagPwrSum=(10*np.log10(DiagPWR_sum));
    Visibility=DiagPWR_sum/(DiagPWR_sum+OffDiagPWR_sum)
    return SNR,DiagPwrSum,Visibility,DiagCoup,OffDiagCoup


# This is the function you would call to do almost any metric stuff it takes 2 sets of modes to construct a coupling matrix
def CalculateCoupMatrixAndMetrics(fieldX,fieldY,pixelsize,UseNorm=False):
    ...
    # CouplingMatrix=np.zeros([modeCount_Out,modeCount_In],dtype=complex)
    # SingularValues=np.zeros(modeCount)
    # DiagCoup=np.zeros(modeCount,dtype=complex)
    # OffDiagCoup=np.zeros(((modeCount_Out*modeCount_In)-modeCount),dtype=complex)
    ...
    # Calculate Coupling Matrix CouplingMatrix is (modeCountY,modeCountX)
    if UseNorm:
        _,CouplingMatrix=CalculateCouplingMatrix_WithNorm(fieldX,fieldY,pixelsize)
    else:
        CouplingMatrix=CalculateCouplingMatrix(fieldX,fieldY,pixelsize)
    
    # print(CouplingMatrix.shape)
    
    # Calculate IL and MDL
    IL,MDL,VolumePres,SingularValues=CalculateILAndMDL(CouplingMatrix)
    
    # Calculate SNR DiagSum and Visibility
    DimsCoup=CouplingMatrix.shape
    if(DimsCoup[0]==DimsCoup[1]):
        SNR,DiagPwrSum,Visibility,DiagCoup,OffDiagCoup=CalculateSNR(CouplingMatrix)
    else:
        print("Coupling matrix Dims are not equal which means you cant determine SNR Visibility or DiagPwrSum from the matrix Directly.\n The coupling matrix will be Multiplyed by its congjuante transpose to fix this.")
        CouplingMatrix_Conj = np.conj(CouplingMatrix.T)
        CouplingMatrix_x_Conj=np.dot(CouplingMatrix_Conj,CouplingMatrix)
        SNR,DiagPwrSum,Visibility,DiagCoup,OffDiagCoup=CalculateSNR(CouplingMatrix_x_Conj)
        print(CouplingMatrix_x_Conj.shape)
        # plt.figure()
        # plt.title("CouplMat*CouplMat^*")
        # plt.imshow(cmplxplt.ComplexArrayToRgb(CouplingMatrix_x_Conj))
        
    overlapSum=np.sum(np.abs(CouplingMatrix)**2)
    outputstr='IL= '+ str(IL) +' MDL= ' +str(MDL) + ' Vis= '+str(Visibility)+ ' sum(diag)=' +str(DiagPwrSum) +' SNR=' +str(SNR) +" OverlapSum=" + str(overlapSum) +" VolumePres=" +str(VolumePres)
    print(outputstr)
    Metrics=AllMetricValues(CouplingMatrix,SingularValues,DiagCoup,OffDiagCoup,IL,MDL,VolumePres,overlapSum,Visibility,DiagPwrSum,SNR)
    return Metrics
def CalculateMetrics(CouplingMatrix):
    
    # class AllMetricValues:
    #     def __init__(self,CouplingMatrix,SingularValues,DiagCoup,OffDiagCoup,IL,MDL,Visibility,DiagPwrSum,SNR):
    #         self.CouplingMatrix=CouplingMatrix
    #         self.SingularValues=SingularValues
    #         self.DiagCoup=DiagCoup
    #         self.OffDiagCoup=OffDiagCoup
