...
import Lab_Equipment.OpticalSimulations.libs.CoupMatrixAndMetricAnalysisFuncitons_old as MetricCals
import Lab_Equipment.OpticalSimulations.libs.GaussianBeamBasis as GaussBeams
import Lab_Equipment.OpticalSimulations.libs.GeneralFunctions as GenFunc
...
def GenerateLGAzthmModes(x,y,z_dist,MFD, wavelen,modeIndeices):
    ...
    TH_vec,R_vec=GenFunc.cart2pol(X_temp, Y_temp)
    TH_vec=TH_vec
    X_temp,Y_temp=GenFunc.pol2cat(TH_vec,R_vec)
    ...
    for imode in range(modeCount):
        ...
        Modes[imode,:,:]=(GaussBeams.GenerateLGMode(MFD, wavelen, p, l, pixelSize,X_temp, Y_temp, z_dist, MAXmaxMG))
