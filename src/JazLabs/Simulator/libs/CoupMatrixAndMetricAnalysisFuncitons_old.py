...
class AllMetricValues:
    def __init__(self,CouplingMatrix,SingularValues,DiagCoup,OffDiagCoup,IL,MDL,Visibility,DiagPwrSum,SNR):
        ...
        self.SNR=SNR
def gram_from_fields(fields, mask=None):
    # fields: (N, Ny, Nx) complex
    N, Ny, Nx = fields.shape
    X = fields.reshape(N, -1)

    if mask is not None:
        w = mask.reshape(-1).astype(float)
        X = X * np.sqrt(w)[None, :]

    # normalise each field vector
    ...
    G = Xn @ Xn.conj().T
    return G
def CalculateGramMatrix(fieldX,fieldY,pixelsize):
    ...
    GramMat_norm=np.zeros((modeCountY,modeCountX),dtype=complex)
    for imodeY in range(modeCountY):
        for imodeX in range(modeCountX):
            ...
            GramMat_norm[imodeY,imodeX]=(np.sum((fieldY[imodeY,:,:])*(np.conj(fieldX[imodeX,:,:])))\
...
def CalculateCoupMatrixAndMetrics(fieldX,fieldY,pixelsize):
