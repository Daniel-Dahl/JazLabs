...
#include "digHolo_arm64_compat.h"
...
int cpuInfoGet(cpuINFO* c)
{
	...
	const char brand[] = "Apple Silicon arm64";
	memcpy(c[0].brand, brand, sizeof(brand));
...
#ifdef LAPACKBLAS_ENABLE
	cgesvd(&jobu, &jobv, &M, &N, (BLAS_COMPLEXTYPE*)a, &LDA, s, (BLAS_COMPLEXTYPE*)U, &LDU, (BLAS_COMPLEXTYPE*)VT, &LDVT, (BLAS_COMPLEXTYPE*)WORK, &LWORK, RWORK, &info, (size_t)1, (size_t)1);
	...
	return 0;
	  ...
	  void threadPoolDestroy()
	  {
		  ...
		  for (int j = 0; j < digHoloThreadPoolSize; j++)
		  {
			  ...
			  if (digHoloThreadPool[j].joinable())
			  {
				  digHoloThreadPool[j].join();
			  }
			  delete workPack;
		  }

		  digHoloWorkPacks.clear();
		  digHoloThreadPool.clear();
	  }
		   ...
		   if (batchCount > 1 || polCount == 2)
		   {
			   ...
			   for (int polIdx = 0; polIdx < polThreads; polIdx++)
			   {
				   ...
				   for (int batchIdx = 0; batchIdx < batchThreads; batchIdx++)
				   {
					   ...
					   //Reset event to Set when thread is complete
					   workPack[0].workCompleteEvent.Reset();
					   runWorkPackage(workPack[0]);
					   workPack[0].workCompleteEvent.Set();
					   //Launch the thread
...
private: void digHoloAutoAlignSnap(unsigned char doFinalOverlap, int polsAreIndependent, float** outputMetrics)
{
	if (config.verbosity >= DIGHOLO_VERBOSITY_DEBUG)
	{
		fprintf(consoleOut, "AutoAlignSnap: start\n\r");
		fflush(consoleOut);
	}
	...
	if (config.verbosity >= DIGHOLO_VERBOSITY_DEBUG)
	{
		fprintf(consoleOut, "AutoAlignSnap: FFT start\n\r");
		fflush(consoleOut);
	}
	digHoloFFT();
	if (config.verbosity >= DIGHOLO_VERBOSITY_DEBUG)
	{
		fprintf(consoleOut, "AutoAlignSnap: FFT done\n\r");
		fflush(consoleOut);
	}
	...
	if (config.verbosity >= DIGHOLO_VERBOSITY_DEBUG)
	{
		fprintf(consoleOut, "AutoAlignSnap: DCT plan start\n\r");
		fflush(consoleOut);
	}
	int success = digHoloDCTGetPlan();
	if (config.verbosity >= DIGHOLO_VERBOSITY_DEBUG)
	{
		fprintf(consoleOut, "AutoAlignSnap: DCT plan done\n\r");
		fflush(consoleOut);
	}
	...
	for (int passIdx = 0; passIdx < passCount; passIdx++)
	{
		...
		for (int polIdx = 0; polIdx < digHoloPolCount; polIdx++)
		{
			...
			//if (passIdx == 0)
			{
				...
				if (config.verbosity >= DIGHOLO_VERBOSITY_DEBUG)
				{
					fprintf(consoleOut, "AutoAlignSnap: DCT execute start\n\r");
					fflush(consoleOut);
				}
				fftwf_execute_r2r(digHoloDCTPlan, in, out);
				if (config.verbosity >= DIGHOLO_VERBOSITY_DEBUG)
				{
					fprintf(consoleOut, "AutoAlignSnap: DCT execute done\n\r");
					fflush(consoleOut);
				}
				...
				if (config.verbosity >= DIGHOLO_VERBOSITY_DEBUG)
				{
					fprintf(consoleOut, "AutoAlignSnap: IDCT execute start\n\r");
					fflush(consoleOut);
				}
				fftwf_execute_r2r(digHoloIDCTPlan, out, out2);
				if (config.verbosity >= DIGHOLO_VERBOSITY_DEBUG)
				{
					fprintf(consoleOut, "AutoAlignSnap: IDCT execute done\n\r");
					fflush(consoleOut);
				}
