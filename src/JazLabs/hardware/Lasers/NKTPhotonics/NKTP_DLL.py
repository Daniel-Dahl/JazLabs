...
#NOTE from Daniel: ok so the SDK is another thing that has to be installed but i am just going to include it in the directory system because fuck NKT and there 
# shitty interface system
WORKING_DIR = os.getcwd()
dllFolder=WORKING_DIR+"\\Lab_Equipment\\Laser\\NKTPhotonics"
# This is the originial line that will find the SDK in the usual install location
...
# \param portname Zero terminated string giving the portname (case sensitive).
# \param devId Given device id to retrieve device type for (module type).
# \param devType Pointer to an unsigned short where the function stores the device type.
# \return A status result value ::DeviceResultTypes
# \note Register address 0x61\n
#       It is not necessary to open the port, create the device or register before using this function, since it will do a dedicated read.
#
#extern "C" NKTPDLL_EXPORT DeviceResultTypes deviceGetTypeV2(const char *portname, const unsigned char devId, unsigned short *devType);
...
_deviceGetTypeV2 = CFUNCTYPE(c_ubyte, c_char_p, c_ubyte, POINTER(c_ushort))(('deviceGetTypeV2', NKTPDLL))
def deviceGetTypeV2(portname, devId):
	_readValue = c_ushort(0)
	result = _deviceGetTypeV2(portname.encode('ascii'), devId, _readValue)
	return result, _readValue.value
...
#       It is not necessary to open the port, create the device or register before using this function, since it will do a dedicated read.
#
#extern "C" NKTPDLL_EXPORT DeviceResultTypes deviceGetSysType(const char *portname, const unsigned char devId, unsigned char *sysType);
...
_deviceGetSysType = CFUNCTYPE(c_ubyte, c_char_p, c_ubyte, POINTER(c_ubyte))(('deviceGetSysType', NKTPDLL))
def deviceGetSysType(portname, devId):
	_readValue = c_ubyte(0)
	result = _deviceGetSysType(portname.encode('ascii'), devId, _readValue)
	return result, _readValue.value
...
# \return A status result value ::DeviceResultTypes
# \note Requires the port being already opened with the ::openPorts function.
...
_deviceGetAllTypes = CFUNCTYPE(c_ubyte, c_char_p, POINTER(c_ubyte), POINTER(c_ubyte))(('deviceGetAllTypes', NKTPDLL))
def deviceGetAllTypes(portname):
	_maxTypes = c_ubyte(255)
	_types = (c_ubyte * _maxTypes.value)()
	result = _deviceGetAllTypes(portname.encode('ascii'), _types, _maxTypes)
	return result, _types

# \brief Returns a list with device types (module types) from the internal devicelist.
# \param portname Zero terminated string giving the portname (case sensitive).
# \param types Pointer to a preallocated area where the function stores the list of module types. The default list size is 512 bytes long (0-255 shorts) where each position indicates module address, containing 0 for no module or the module type for addresses having a module.\n
#  ex. 0000h 0061h 0062h 0063h 0064h 0065h 0000h 0000h 0000h 0000h 0000h 0000h 0000h 0000h 0000h 0060h 0000h 0000h etc.\n
# Indicates module type 0061h at address 1, module type 0062h at address 2 etc. and module type 0060h at address 15
# \param maxTypes Pointer to an unsigned char giving the maximum number of types to retrieve.
#                 The returned list may be truncated to fit into the allocated area.
# \return A status result value ::tDeviceResultTypes
# \note Requires the port being already opened with the ::openPorts function.
#
#extern "C" NKTPDLL_EXPORT DeviceResultTypes deviceGetAllTypesV2(const char *portname, unsigned short *types, unsigned char *maxTypes);
...
_deviceGetAllTypesV2 = CFUNCTYPE(c_ubyte, c_char_p, POINTER(c_ushort), POINTER(c_ubyte))(('deviceGetAllTypesV2', NKTPDLL))
def deviceGetAllTypesV2(portname):
	_maxTypes = c_ubyte(25)
	_types = (c_ushort * _maxTypes.value)()
	result = _deviceGetAllTypesV2(portname.encode('ascii'), _types, _maxTypes)
	return result, _types
...
#                                                  const unsigned short foundType); // device found type
portStatusCallbackFuncPtr = CFUNCTYPE(None, c_char_p, c_ubyte, c_ubyte, c_ubyte, c_ushort)
...
#print("openResult = openPorts(\"COM25\", 1, 1)")
#openResult = openPorts("COM25", 1, 1)
#print("openResult:", openResult, PortResultTypes(openResult) )

#print("print(getOpenPorts())")
#print(getOpenPorts())

#print("print(deviceGetType())")
#print(deviceGetType("COM25", 7))

#print("print(deviceGetTypeV2())")
#print(deviceGetTypeV2("COM25", 7))

#print("print(deviceGetAllTypes())")
#gatResult, typeList = deviceGetAllTypes("COM25")
#print(DeviceResultTypes(gatResult))
#for devId in range(0, len(typeList)):
#        if (typeList[devId] != 0):
#            print('Device type:',"0x%0.2X" % typeList[devId],'at address:',devId)

#print("print(deviceGetAllTypesV2())")
#gatResult, typeList = deviceGetAllTypesV2("COM25")
#print(DeviceResultTypes(gatResult))
#for devId in range(0, len(typeList)):
#        if (typeList[devId] != 0):
#            print('Device type:',"0x%0.4X" % typeList[devId],'at address:',devId)
