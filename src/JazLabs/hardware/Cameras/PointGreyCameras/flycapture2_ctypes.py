...
class FlyCapture2Library:
    ...
    def _set_prototypes(self):
        ...
        lib.fc2GetConfiguration.argtypes = [ctypes.c_void_p, ctypes.POINTER(fc2Config)]
        lib.fc2GetConfiguration.restype = ctypes.c_int

        lib.fc2SetConfiguration.argtypes = [ctypes.c_void_p, ctypes.POINTER(fc2Config)]
        lib.fc2SetConfiguration.restype = ctypes.c_int
        ...
        lib.fc2ReadRegister.argtypes = [
            ctypes.c_void_p,
            ctypes.c_uint,
            ctypes.POINTER(ctypes.c_uint),
        ]
        lib.fc2ReadRegister.restype = ctypes.c_int

        lib.fc2WriteRegister.argtypes = [
            ctypes.c_void_p,
            ctypes.c_uint,
            ctypes.c_uint,
        ]
        lib.fc2WriteRegister.restype = ctypes.c_int
    ...
    def get_configuration(self, ctx):
        config = fc2Config()
        _check_error(
            self.lib.fc2GetConfiguration(ctx, ctypes.byref(config)),
            "fc2GetConfiguration",
        )
        return config

    def set_configuration(self, ctx, config):
        _check_error(
            self.lib.fc2SetConfiguration(ctx, ctypes.byref(config)),
            "fc2SetConfiguration",
        )
    ...
    def fire_software_trigger(self, ctx):
        code = int(self.lib.fc2FireSoftwareTrigger(ctx))
        _check_error(code, "fc2FireSoftwareTrigger")
        return code

    def read_register(self, ctx, address):
        value = ctypes.c_uint()
        _check_error(
            self.lib.fc2ReadRegister(ctx, ctypes.c_uint(address), ctypes.byref(value)),
            "fc2ReadRegister",
        )
        return int(value.value)

    def write_register(self, ctx, address, value):
        code = int(
            self.lib.fc2WriteRegister(
                ctx,
                ctypes.c_uint(address),
                ctypes.c_uint(value),
            )
        )
        _check_error(code, "fc2WriteRegister")
        return code
