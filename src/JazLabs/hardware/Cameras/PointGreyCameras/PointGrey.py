...
from .flycapture2_ctypes import (
    FlyCapture2Library,
    FlyCapture2Error,
    fc2PropertyType,
    fc2Mode,
...
class CameraObject:
    ...
    def DrainImageBuffer(self, max_frames=64, timeout_ms=1):
        """
        Drain frames already queued inside FlyCapture2.

        ResetBuffer() only recreates this wrapper's fc2Image object; it does not
        empty frames that the SDK/driver has already queued. Use a short grab
        timeout and retrieve until FlyCapture2 reports no frame is ready.
        """
        if not self._capturing:
            return 0
        ...
        drained = 0
        drain_image = self.fc2.create_image()

        try:
            self.SetGrabTimeout(timeout_ms)
            for _ in range(int(max_frames)):
                try:
                    self.fc2.retrieve_buffer(self.context, drain_image)
                    drained += 1
                except FlyCapture2Error:
                    break
        finally:
            try:
                self.fc2.destroy_image(drain_image)
            except Exception:
                pass
    ...
    def GetGrabTimeout(self):
        return int(self.fc2.get_configuration(self.context).grabTimeout)

    def SetGrabTimeout(self, timeout_ms):
        """
        Set the FlyCapture2 RetrieveBuffer timeout in milliseconds.

        Use -1 for the SDK's infinite wait behaviour.
        """
        config = self.fc2.get_configuration(self.context)
        config.grabTimeout = int(timeout_ms)
        self.fc2.set_configuration(self.context, config)
        return self.GetGrabTimeout()

    def IsSoftwareTriggerReady(self):
        """
        Return True when the FLIR software trigger register is ready to fire.
        """
        software_trigger_register = 0x62C
        register_value = self.fc2.read_register(self.context, software_trigger_register)
        return (register_value >> 31) == 0

    def WaitForSoftwareTriggerReady(self, timeout_ms=1000, poll_interval_s=0.001):
        """
        Wait until the camera reports that another software trigger can be fired.
        """
        ...
        while True:
            ...
            if timeout_ms is not None and time.perf_counter() >= deadline:
                raise TimeoutError(
                    f"Timed out waiting {timeout_ms} ms for FLIR software trigger ready"
                )
    ...
    def FireSoftwareTrigger(self, wait_ready=True, ready_timeout_ms=1000, drain_stale_frames=True):
        ...
        return self.fc2.fire_software_trigger(self.context)
