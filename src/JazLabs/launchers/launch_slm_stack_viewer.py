...
def start_slm_stack_viewer(
    ...
    server = config.get("SLM_STACK_SERVER", {})
    viewer = merge_overrides(
        config.get("SLM_STACK_VIEWER", {}),
        {
            "host": host,
    ...
    viewer.setdefault("window_name", "SLMStack viewer")
    ...
    return [("slm stack viewer", process)]
...
def main(argv=None):
    ...
    mp.freeze_support()
    processes = start_slm_stack_viewer(
