...
def build_parser():
    parser = argparse.ArgumentParser(description="Run the JazLabs SLM server.")
...
def main(argv=None):
    ...
    if "SLM_SERVER" not in config and "SLM_MILK_SERVER" in config:
        raise ValueError(
            f"Config {args.config!r} defines SLM_MILK_SERVER, not SLM_SERVER. "
            "Launch it with jazlabs-server-slm-milk."
        )
    ...
    slm.setdefault("name", "slm_server")
    ...
    print(f"Launching SLM server {slm['name']!r}.")
    ...
    try:
        ...
        print("SLMStack server interrupted.")
