...
def main(argv=None):
    ...
    if "SLM_MILK_SERVER" not in config:
        if "SLM_SERVER" in config:
            raise ValueError(
                f"Config {args.config!r} defines SLM_SERVER, not "
                "SLM_MILK_SERVER. Launch it with jazlabs-server-slm."
            )
        raise ValueError(f"Config {args.config!r} does not define SLM_MILK_SERVER.")

    slm = merge_overrides(
        config["SLM_MILK_SERVER"],
        {
            "host": args.host,
